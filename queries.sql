-- /Users/andyvungoc/TDA553/Databases-DBM1/queries.sql
...
-- 4) (More complex) Album with most tracks per genre
...
ranked AS (
  SELECT
    album_counts.*,
    RANK() OVER (PARTITION BY genre_name ORDER BY num_tracks DESC, title ASC) AS rnk
  FROM album_counts
)
SELECT
  genre_name,
